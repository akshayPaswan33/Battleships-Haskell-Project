...
valid_seq :: Int -> String -> Int -> Bool
valid_seq a m x = valid_list [ valid_ind y m|y <- [x..(x+a-1)]]

compute_prob'' :: Int -> String -> Int -> [Int]
compute_prob'' a m x =  if valid_seq a m x
...
compute_prob' ::Int -> String -> [Int] -> [Int]
compute_prob' a m xs =  myadd [compute_prob'' a m x | x <- xs]

compute_prob :: Int -> String -> [Int] -> [Int]
compute_prob a m [] = []
compute_prob a m xs = (compute_prob' a m (take 11 xs)) ++ (compute_prob a m (drop 11 xs))

chunk [] = []
chunk xs = take 11 xs : (chunk (drop 11 xs))

compute_final_prob :: Int -> String -> [Int]
compute_final_prob a m = zipWith (+) (compute_prob a m [0..120]) (concat (transpose (chunk (compute_prob a (concat (transpose (chunk m))) [0..120]))))

compute_hunt_prob :: [(Int,Int)] -> String -> [Int]
compute_hunt_prob xs m = myadd [ compute_final_prob a m|(a,b)<-xs,b==0]

ind_prob_pair :: [(Int,Int)] -> String -> [(Int,Int)]
ind_prob_pair xs m = zip [0..120] (compute_hunt_prob xs m)

mysort :: [(Int,Int)] -> [(Int,Int)]
mysort [] = []
mysort (x:xs) = mysort ([ y | y <- xs, (snd y) >= (snd x)]) ++ [x] ++ mysort ([ y | y <- xs, (snd y) < (snd x)])

hunt :: [(Int,Int)] -> String -> IO (Int,Int)
hunt xs m = do
                t <- return(fst(head (mysort (ind_prob_pair xs m))))
...
comp_play :: String -> Int -> Int -> (Int,Int) -> (Int,Int) -> Int -> [(Int,Int)] -> IO (Char,Char)
comp_play m hunt_flag hit_flag last_hit last_miss sunk xs   =   if hunt_flag == 1
                                                                then    do
                                                                            (c1,c2) <- hunt xs m
...
playgame_mid :: (String,String)->[[((Char,Char),Int)]]->[[((Char,Char),Int)]]->String->String->String->String->Int->Int->Int->(Int,Int)->(Int,Int)->Int->Int->[(Int,Int)]->IO ()
playgame_mid (p1,p2) pos1 pos2 f1 f2 g1 g2 c1 c2 hf lh lm sunk hunt_flag xs =  if c1 == 17
                                                                               then  if p1 == "comp"
                                                                                     then do
                                                                                             putStrLn ("\n"++p1++" wins")
                                                                                             putStrLn "comp map"
                                                                                      ...
                                                                                      box <-if p1=="comp"
                                                                                            then comp_play g2 hunt_flag' hf lh lm sunk xs
                                                                                            else valid_box_attack g2
                                                                                      if p1 == "comp"
                                                                                      then do
                                                                                     ...
                                                                                     --putStrLn ""
                                                                                      ...
                                                                                      putStrLn ""
                                                                                      if strike then playgame_mid (p1,p2) pos1 pos2' f1 f2 g1 g2' (c1+1) c2 hf lh' lm' sunk' hunt_flag' xs' else playgame_mid (p2,p1) pos2' pos1 f2 f1 g2' g1 c2 c1 hf lh' lm' sunk' hunt_flag' xs'
...
main = do
            ...
            playgame_mid players init_positions_list_p1 pos (h (concat coordinatesOfShips_p1)) (h (convert_pos_field pos)) (h []) (h []) 0 0  0 (-1,-1) (-1,-1) 0 1 [(5,0),(4,0),(3,0),(3,0),(2,0)]
